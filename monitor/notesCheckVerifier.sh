...
function __sendMail {
 ...
 if [[ "${QTY}" -ne 0 ]]; then
  ...
  echo "" | mutt -s "OSM Notes profile differences" -i "${REPORT}" -a "${REPORT_ZIP}" -- "${EMAILS}" >> "${LOG_FILE}"
...
function __cleanFiles {
 ...
 if [[ "${CLEAN_FILES}" = "true" ]]; then
  ...
  rm -f "${REPORT}" "${REPORT_ZIP}" /tmp/lastNote.csv /tmp/lastNoteComment.csv \
   /tmp/differentNoteIds.csv /tmp/differentNoteCommentIds.csv \
   /tmp/differentNotes.csv /tmp/differentNoteComments.csv
 fi
 __log_finish
}
