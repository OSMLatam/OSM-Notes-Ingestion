...
@test "Edge case: Empty database should be handled gracefully" {
 ...
 if [[ "${CI:-}" == "true" ]] || [[ "${GITHUB_ACTIONS:-}" == "true" ]]; then
  ...
  # but verify that the psql command is available
  run command -v psql
  [[ "${status}" -eq 0 ]]

  echo "Skipping actual database operations in CI environment - psql command validated"
  return 0
 fi
 ...
 if [[ -n "${TEST_DBHOST}" ]]; then
  ...
  run psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT COUNT(*) FROM information_schema.tables;"
 else
  # Local connection
...
@test "Edge case: Corrupted database should be handled gracefully" {
 ...
 if [[ -n "${TEST_DBHOST}" ]]; then
  # Remote connection
  # shellcheck disable=SC2154,SC2153
  run psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT * FROM nonexistent_table;"
 else
  # Local connection
...
@test "Edge case: Concurrent access should be handled gracefully" {
 ...
 (
  if [[ -n "${TEST_DBHOST}" ]]; then
   # Remote connection
   psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT 1;" &
   psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT 2;" &
  else
   # Local connection
 ...
 if [[ -n "${TEST_DBHOST}" ]]; then
  # Remote connection
  # shellcheck disable=SC2154,SC2153
  run psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT 3;"
 else
  # Local connection
...
@test "Edge case: Data corruption should be handled gracefully" {
 ...
 if [[ -n "${TEST_DBHOST}" ]]; then
  # Remote connection
  # shellcheck disable=SC2154,SC2153
  run psql -h "${TEST_DBHOST}" -p "${TEST_DBPORT}" -U "${TEST_DBUSER}" -d "${TEST_DBNAME}" -c "SELECT 'invalid'::integer;"
 else
  # Local connection
