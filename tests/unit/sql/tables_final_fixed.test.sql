-- Unit tests for database tables (final version without pgTAP)
...
-- Test 19: Test data insertion into note_comments table (with proper foreign key handling)
DO $$
DECLARE
...
BEGIN
  ...
  -- Get or create a user for the foreign key constraint
  SELECT users.user_id INTO test_user_id FROM users LIMIT 1;
  IF test_user_id IS NULL THEN
    -- Create a test user if none exists
    INSERT INTO users (user_id, username) VALUES (123, 'test_user') ON CONFLICT DO NOTHING;
    test_user_id := 123;
  END IF;
  
  -- Insert test data
  INSERT INTO note_comments (note_id, sequence_action, event, created_at, id_user) 
  VALUES (999, 1, 'opened', NOW(), test_user_id);
...
-- Test 20: Test data insertion into note_comments_text table (with proper foreign key handling)
DO $$
DECLARE
  text_count INTEGER;
BEGIN
