...
if [[ -n "${API_NOTES_FILE:-}" && -f "${API_NOTES_FILE}" ]]; then
 ...
 xmlstarlet sel -t -m "//note" -v "@lat" -o "," -v "@lon" -o "," -v "id" -o "," -v "status" -o "," -v "date_created" -n "${API_NOTES_FILE}" | while IFS=',' read -r lat lon note_id status created_at; do
 ...
 xmlstarlet sel -t -m "//comment" -v "ancestor::note/id" -o "," -v "date" -o "," -v "uid" -o "," -v "action" -o "," -v "text" -n "${API_NOTES_FILE}" | while IFS=',' read -r note_id date uid action text; do
